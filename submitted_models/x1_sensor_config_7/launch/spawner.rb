...
def rosExecutables(_name, _worldName)
  ...
  <executable name='robot_description'>
    <command>roslaunch --wait x1_sensor_config_7 description.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  <executable name='topics'>
    <command>roslaunch --wait x1_sensor_config_7 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  HEREDOC
end
